...
window.onload = function () {
    ...
    // Bloco de segurança para o cover-album
    var coverArt = document.getElementsByClassName('cover-album')[0];
    if (coverArt) { // Só executa se o elemento existir
        ...
        function applyCover(url) {
            ...
            if (coverBackground) {
                ...
                img.onload = () => {
                    ...
                    // 2. ESPERA A ANIMAÇÃO ACABAR E TROCA A IMAGEM
                    coverBackground.addEventListener('animationend', function onOutEnd(e) {
                        if (e.animationName === 'clipOut') {
                            ...
                            // 3. TROCA A IMAGEM DE FUNDO
                            coverBackground.style.backgroundImage = `url(${url})`;

                            // Simplesmente dispare o evento toda vez que a capa for aplicada.
                            document.dispatchEvent(new Event('coverArtLoaded'));
                            console.log("Evento 'coverArtLoaded' disparado!");
...
window.syncPlayerUI = syncPlayerUI;